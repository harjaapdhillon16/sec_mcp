...
const errorResponse = (logger, message, code = null, details = null, extraContent = []) => {
  const payload = buildErrorPayload(message, code, details);
  const response = {
    content: [
      { type: 'text', text: message || 'Error' },
      ...extraContent
    ],
...
const buildTextContent = (value) => {
  ...
  if (value.status === 'error' && value.error?.message) return String(value.error.message);
  return 'OK';
};
...
const jsonResponse = (logger, structuredContent, extraContent = []) => {
  ...
  if (!isPlainObject(normalized)) {
    return errorResponse(logger, 'Invalid structured content produced by tool', 'invalid_structured_content', null, extraContent);
  }
  const textContent = buildTextContent(normalized);
  const content = textContent ? [{ type: 'text', text: textContent }, ...extraContent] : [...extraContent];
  const response = {
    content,
...
const handleLatestIntel = async (args, extra, logger) => {
  ...
  try {
    ...
    if (!company || !filing) {
      ...
      return jsonResponse(logger, fallback.intel, [{ type: 'text', text: sourcesText }]);
    }
...
const handleEarningsIntel = async (args, logger) => {
